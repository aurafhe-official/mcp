...
try {
  ...
  const overview = await tool('aura_start')
  assert.equal(overview.connection.status, 'not-checked')
  assert.equal(overview.applications.flagship.model, 'GPT-OSS-20B')
  assert.equal(overview.applications.flagship.availableThroughThisMcp, false)
  assert.equal(overview.applications.flagship.benchmark.liveMeasurement, false)
  const welcome = await tool('aura_start', { experience: 'learn' })
  assert.equal(welcome.smokeTest.status, 'not-run')
  assert.equal(welcome.guide.step, 1)
  assert.equal(welcome.guide.nextAction.tool, 'fhe_inputs')
  ...
  const prepared = await tool(welcome.guide.nextAction.tool, welcome.guide.nextAction.arguments)
  const { inputs } = prepared
  assert.equal(prepared.guide.step, 2)
  assert.deepEqual(prepared.guide.nextAction, { tool: 'fhe_compute', arguments: { op: 'add', handles: inputs.slice(0, 2).map(i => i.handle) } })
  ...
  async function verify(op, indexes, domain, expected, supplied) {
    ...
    assert.equal(computed.guide.step, 3)
    assert.deepEqual(computed.guide.nextAction, { tool: 'fhe_export', arguments: { handle: computed.handle } })
    const exported = await tool(computed.guide.nextAction.tool, computed.guide.nextAction.arguments)
    assert.equal(exported.guide.step, 4)
