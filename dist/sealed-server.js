...
import { InputBundle, MAX_BUNDLE_BYTES, SealedSession, computeClient } from './sealed.js';
const Handle = z.string().regex(/^ct_[a-f0-9]{32}$/);
const answer = (value, isError = false) => ({ content: [{ type: 'text', text: JSON.stringify(value) }], isError });
export function createSealedServer(session, save) {
    ...
    const ready = () => { if (!session)
        throw new Error('Owner bundle and compute worker must be configured before computation'); return session; };
    async function run(work) {
        try {
            return answer(await work());
        }
        catch (err) {
            return answer({ error: err instanceof Error ? err.message : 'Operation failed' }, true);
        }
    }
    ...
    server.registerTool('fhe_export_result', {
        ...
        if (!save)
            throw new Error('Owner result destination is not configured');
...
export async function configuredSealedServer() {
    ...
    if (!bundlePath && !workerUrl && !keyId)
        return createSealedServer();
    if (!bundlePath || !workerUrl || !keyId)
        throw new Error('Set AFHE_INPUT_BUNDLE, AFHE_COMPUTE_URL, and AFHE_KEY_ID together');
    if (!isAbsolute(bundlePath) || (await stat(bundlePath)).size > MAX_BUNDLE_BYTES)
        throw new Error('Input bundle must be an absolute path within the size limit');
    const bundle = InputBundle.parse(JSON.parse(await readFile(bundlePath, 'utf8')));
    if (bundle.keyId !== keyId)
        throw new Error('Owner bundle key identifier does not match the configured worker');
    const worker = computeClient(workerUrl, process.env.AFHE_COMPUTE_API_KEY);
    if ((await worker.health()).keyId !== keyId)
        throw new Error('Worker key identifier does not match the owner bundle');
    ...
    if (destination && !isAbsolute(destination))
        throw new Error('AFHE_RESULT_DIR must be an absolute path');
