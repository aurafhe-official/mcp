...
async function main() {
    ...
    if (!input || (action !== 'prepare' && action !== 'decrypt') || (action === 'prepare' && !output))
        throw new Error('Usage: aura-owner prepare plaintext.json encrypted.json | aura-owner decrypt encrypted-result.json');
    const ownerUrl = process.env.AFHE_OWNER_URL, workerUrl = process.env.AFHE_COMPUTE_URL, keyId = process.env.AFHE_KEY_ID;
    if (!ownerUrl || !workerUrl || !keyId)
        throw new Error('Configure AFHE_OWNER_URL, AFHE_COMPUTE_URL, and AFHE_KEY_ID on the owner device');
    if ((await stat(input)).size > MAX_BUNDLE_BYTES)
    ...
    if (action === 'prepare') {
        if (!value || !Array.isArray(value.values))
            throw new Error('Input must contain domain and values');
