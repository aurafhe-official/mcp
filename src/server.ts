...
export function createFheServer(session: FheSession) {
  async function safe(fn: () => Promise<object>) {
    ...
    catch (e) { return { isError: true, content: [{ type: 'text' as const, text: JSON.stringify({ ...session.context(), error: e instanceof AuraError ? e.code : 'COPROCESSOR_REQUEST_FAILED' }) }] } }
  }
  ...
  server.registerTool('aura_start', { description: 'Read this first: explain the current mode and FHE base layer, check connectivity and list usable operations. Does not run or claim a verified smoke test.', inputSchema: z.strictObject({}), annotations: { readOnlyHint: true } },
    async (_, ctx) => safe(() => session.start(ctx.mcpReq.signal)))
  server.registerTool('aura_roadmap', { description: 'Describe available numeric primitives, application compositions, planned binary support and the production client/server pattern. Contact Aura for separate application demonstrations.', inputSchema: z.strictObject({}), annotations: { readOnlyHint: true } },
