...
import { AuraError, Computed, Exported, Imported, KeyRef, MAX_INPUTS, ObjectRef, OpaqueId,
  SessionInfo, sameKey, supported, type Operation } from './contracts.js'
...
export class FheSession {
  ...
  private handles = new Map<string, ObjectRef>()
  private busy = false
  private calls: number[] = []
  constructor(private remote: Coprocessor, private key: KeyRef, private now = () => Date.now()) {}
  ...
  private async ensure(signal?: AbortSignal) {
    ...
    if (!this.session) {
      ...
      if (!sameKey(session.key, this.key) || session.expiresAt <= this.now() || session.expiresAt > this.now() + 24 * 60 * 60_000 || session.capabilities.some(c => !supported(c))) throw new AuraError('INVALID_COPROCESSOR_SESSION')
      if (new Set(session.capabilities.map(c => `${c.domain}/${c.op}`)).size !== session.capabilities.length) throw new AuraError('INVALID_COPROCESSOR_SESSION')
  ...
  private scope(value: { sessionId: string; key: KeyRef }) {
    if (value.sessionId !== this.session?.sessionId || !sameKey(value.key, this.key)) throw new AuraError('COPROCESSOR_SCOPE_MISMATCH')
  }
  private validateRefs(refs: ObjectRef[]) {
    if (refs.some(r => r.expiresAt <= this.now() || r.expiresAt > this.session!.expiresAt)) throw new AuraError('INVALID_OBJECT_EXPIRY')
    if (this.handles.size + refs.length > 2048) throw new AuraError('HANDLE_LIMIT')
  }
  private remember(ref: ObjectRef) {
  ...
  async compute(op: Operation, handles: string[], signal?: AbortSignal) {
    return this.exclusive(async () => {
      const scope = await this.ensure(signal)
      if (!handles.length || handles.length > MAX_INPUTS) throw new AuraError('INVALID_OPERATION')
      ...
      if (!cap || handles.length < cap.minInputs || handles.length > cap.maxInputs) throw new AuraError('INVALID_OPERATION')
      if (this.handles.size >= 2048) throw new AuraError('HANDLE_LIMIT')
      ...
      if (result.object.domain !== domain) throw new AuraError('INVALID_RESULT_DOMAIN')
      if (signal?.aborted) throw new AuraError('CANCELLED')
  ...
  async exportResult(handle: string, signal?: AbortSignal) {
    return this.exclusive(async () => {
      const scope = await this.ensure(signal)
      ...
      this.scope(result)
      if (result.expiresAt <= this.now() || result.expiresAt > Math.min(this.session!.expiresAt, ref.expiresAt)) throw new AuraError('INVALID_RESULT_EXPIRY')
      return { resultId: result.resultId, domain: ref.domain, expiresAt: result.expiresAt }
    })
  }
  async release(handles: string[], signal?: AbortSignal) {
    return this.exclusive(async () => {
      const scope = await this.ensure(signal)
      ...
      this.scope(result)
      if (result.released !== refs.length) throw new AuraError('INVALID_RELEASE_RESULT')
      for (const h of unique) this.handles.delete(h)
