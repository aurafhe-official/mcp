...
test('application overview works offline and keeps website claims separate from session observations',async()=>{
  ...
  try {
    ...
    assert.equal(flagship.access.browserApplication,'https://chat.afhe.io')
    assert.equal(flagship.access.availability,'Not checked by this tool')
    assert.equal(overview.applications.mcpPurpose.setup.oneClickPrivateIntegration,false)
    assert.match(overview.applications.mcpPurpose.setup.wasm,/not bundled/)
    assert.deepEqual(overview.applications.mcpPurpose,f.session.roadmap().integration)
